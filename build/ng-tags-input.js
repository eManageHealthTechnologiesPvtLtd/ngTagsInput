 ...
 * License: MIT
 *
 * Generated at 2018-09-07 18:56:12 +0530
 */
(function (angular$1) {
'use strict';
    ...
    link: function link(scope, element, attrs, ngModelCtrl) {
      ...
      scope.eventHandlers = {
        input: {
          ...
          editBlur: function editBlur($event, tag) {
        ...
        tag: {
          ...
          dblclick: function dblclick(tag) {
      ...
      events.on('tag-added', scope.onTagAdded).on('invalid-tag', scope.onInvalidTag).on('tag-removed', scope.onTagRemoved).on('tag-clicked', scope.onTagClicked).on('tag-dblclicked', function (tag) {
        ...
        var shouldEditLastTag = key === tiConstants.KEYS.backspace && scope.newTag.text().length === 0 && options.enableEditingLastTag && !scope.isEditing;
...
SelectallDirective.$inject = ['$timeout', '$parse'];
/**
 * @ngdoc directive
 * @name tiSelectall
 * @module ngTagsInput
...
function SelectallDirective($timeout, $parse) {
  'ngInject';
